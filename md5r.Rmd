...
Our message in bits is now (`r msg_bits`) and the length of the message in bits is `r length(msg_bits)`.
...
Now we need to append an additional 64 bits which encode the length of the original message in bits (`r n_orig_bits`), lowest byte first.
...
```
Our message in bits is now (`r msg_bits`) and the length of the message in bits is `r length(msg_bits)`. The modulo of the message length and 512 is `r length(msg_bits) %% 512`.  We are now good to go to the next step - the message processing.
